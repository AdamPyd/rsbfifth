    ...
    return (
        <div style={{ padding: 24 }}>
            ...
            <Card title="调试信息" style={{ marginTop: 24 }}>
                <p><strong>当前环境:</strong> {process.env.MODE}</p>
                <p><strong>API 基地址:</strong> {ACTUAL_BASE_URL}:{process.env.REACT_APP_BACKEND_PORT}{process.env.REACT_APP_API_BASE_PATH}</p>
                <p><strong>后端端口:</strong> {process.env.REACT_APP_BACKEND_PORT}</p>
                <p><strong>前端端口:</strong> {process.env.REACT_APP_WEB_URL_PORT}</p>
            </Card>
        </div>
    );
};
